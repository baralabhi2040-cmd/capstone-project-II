...
from app.services.demo_data import demo_stats
...
def get_stats(
    ...
    try:
        ...
        db.rollback()
        logger.warning("Stats fallback served because live stats failed: %s", exc)
        stats = demo_stats(scope=f"{scope}-demo")
        stats["average_threat_score"] = 70.2
        stats["verification_required_for_snapshots"] = True
        return stats
